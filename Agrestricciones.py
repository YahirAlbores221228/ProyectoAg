...
def cruce(padre1, padre2, miembros):
    ...
    for i in range(num_miembros):
        for dia in range(num_dias):
            for franja in range(num_franjas):
                if i < punto_cruce:
                    tarea = padre1.asignacion[i][dia][franja]
                else:
                    tarea = padre2.asignacion[i][dia][franja]

                # Validación: asegurarse de que la tarea asignada sea válida
                if tarea:
                    miembro = i
                    if tarea.nombre in miembros[miembro].preferencias and \
                       miembros[miembro].habilidades.get(tarea.nombre, 0.5) > 0 and \
                       miembros[miembro].disponibilidad[dia * 24 + franja]:
                        hijo.asignacion[i][dia][franja] = tarea
                    else:
...
def mutacion(planificacion, tasa_mutacion, tareas, miembros):
    num_miembros = len(planificacion.asignacion)
    num_dias = len(planificacion.asignacion[0])
    num_franjas = len(planificacion.asignacion[0][0])
    
    for i in range(num_miembros):
        for dia in range(num_dias):
            for franja in range(num_franjas):
                if random.random() < tasa_mutacion:
                    # Si el miembro está disponible, asignar una tarea aleatoria
                    if miembros[i].disponibilidad[dia * 24 + franja]:
                        # Eliminar tarea actual
                        planificacion.asignacion[i][dia][franja] = None
                        # Asignar una nueva tarea aleatoria
                        tarea = random.choice([None] + tareas)
                        if tarea is not None:
                            # Verificar si el miembro cumple con las preferencias y habilidades para la tarea
                            if tarea.nombre in miembros[i].preferencias and \
                               miembros[i].habilidades.get(tarea.nombre, 0.5) > 0:
                                planificacion.asignacion[i][dia][franja] = tarea
                            else:
                                planificacion.asignacion[i][dia][franja] = None
...
def algoritmo_genetico(miembros, tareas, dias, franjas_horarias, tamano_poblacion, generaciones, tasa_mutacion):
    ...
    for _ in range(generaciones):
        ...
        while len(nueva_poblacion) < tamano_poblacion:
            ...
            hijo = cruce(padre1, padre2, miembros)
